#!/bin/zsh
# Outputs bob-deinterlaced access files using bwdif=mode=send_field
# AND interlaced-preserving ProRes 422 mezzanine files
...
generate_mezzanine_prores() {
  local mkv_file="$1"
  local title_num="$2"
  local output_prefix="$3"
  local mezz_file="${output_prefix}-MZ${title_num}.mov"
  ...
  echo "Creating mezzanine ProRes 422 for $(basename "$mkv_file")..."
  ...
  # Determine if source is progressive
  ...
  case "$field_order" in
    "progressive")
      ...
      ;;
    "tt"|"tb")
      echo "Detected top-field-first interlaced"
      ;;
    "bb"|"bt")
      echo "Detected bottom-field-first interlaced"
      ;;
    "")
      echo "Empty field order, assuming interlaced for safety"
      ;;
    *)
      echo "Unrecognized field order '$field_order', assuming interlaced"
      ;;
  esac

  # Simple ProRes encoding - let ffmpeg handle aspect ratio and interlacing automatically
  if [[ "$is_progressive" == true ]]; then
    echo "Creating progressive ProRes 422."
    if ! ffmpeg -hide_banner -y -i "$mkv_file" \
      -map 0:v -map 0:a -map_chapters 0 -map -0:s \
      -c:v prores_ks -profile:v 2 -pix_fmt yuv422p10le \
      -c:a pcm_s16le \
      "$mezz_file"; then
      echo "ERROR: ffmpeg failed to create mezzanine file"
      return 1
    fi
  else
    echo "Creating interlaced ProRes 422."
    if ! ffmpeg -hide_banner -y -i "$mkv_file" \
      -map 0:v -map 0:a -map_chapters 0 -map -0:s \
      -c:v prores_ks -profile:v 2 -pix_fmt yuv422p10le \
      -flags +ildct+ilme \
  ...
  echo "Finished processing: $mezz_file"
}
...
create_files_from_iso() {
  echo "Entered create_files_from_iso with create_mezzanine=$1, create_access=$2"
  local create_mezzanine="$1"
  local create_access="$2"
  setopt nullglob
  ...
  if [[ -n "$ISO_PATH" && -f "$ISO_PATH" ]]; then
    ...
    echo "Entered create_files_from_iso (create_mezzanine=$create_mezzanine, create_access=$create_access)"
  ...
  # Process renamed files: optionally create mezzanine ProRes and/or access MP4s
  for mkv_file in "${renamed_files[@]}"; do
    ...
    if [[ "$create_mezzanine" == "true" ]]; then
      generate_mezzanine_prores "$mkv_file" "$title_num" "$output_prefix" || echo "WARNING: Failed to create mezzanine ProRes for $mkv_file"
    fi
...
create_access_files_only() {
    ...
    for mkv_file in "$rip_dir"/*.mkv; do
        ...
        base_name=$(basename "$mkv_file" .mkv)
        # Support PM (preservation master), A (access), and MZ (mezzanine) naming patterns
        title_num=$(echo "$base_name" | grep -oE '(PM|A|MZ)[0-9]{2}' | grep -oE '[0-9]{2}')
...
case "$yn" in
    ...
    [Nn]* )
        ...
        echo "1. Exit this script (Ctrl+C or select option 6 from the menu)"
...
while true; do
  ...
  echo "2. Create archival MKV, mezzanine ProRes 422 and access MP4 files"
  echo "3. Create archival MKV and access MP4 files"
  echo "4. Create archival MKVs only"
  echo "5. Create access files only"
  echo "6. Exit"
  read -r choice
  case "$choice" in
    1) create_iso ;;
    2) create_files_from_iso "true" "true" ;;
    3) create_files_from_iso "false" "true" ;;
    4) create_files_from_iso "false" "false" ;;
    5) create_access_files_only ;;
    6) handle_exit ;;
