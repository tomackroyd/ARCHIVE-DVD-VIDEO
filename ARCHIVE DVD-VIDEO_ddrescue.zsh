...
create_iso() {
  ...
  diskutil unmountDisk /dev/$dvd_dev
  ...
  dvd_size_bytes=$(
    diskutil info "$dvd_dev" | grep "Total Size" | cut -d "(" -f2 | awk '{print $1}'
  )
  if [ -z "$dvd_size_bytes" ]; then
    dvd_size_bytes=$(
      diskutil info "$dvd_dev" | grep "Disk Size" | cut -d "(" -f2 | awk '{print $1}'
    )
  fi
  echo "Detected DVD size: $dvd_size_bytes bytes"
  echo "Copying DVD to $iso_path. This may take a while..."
  sudo ddrescue -r3 -s "$dvd_size_bytes" "/dev/$dvd_raw_dev" "$iso_path" "$iso_path.log"
