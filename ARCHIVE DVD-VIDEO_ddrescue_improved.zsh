...
create_iso() {
  ...
  if ! sudo ddrescue -r3 -s "$dvd_size_bytes" "/dev/$dvd_raw_dev" "$iso_path" "$iso_path.log"; then
...
create_files_from_iso() {
  ...
  # Process renamed files: create JSON metadata and optionally access MP4s
  for mkv_file in "${renamed_files[@]}"; do
    ...
    if ! ffprobe -v quiet -print_format json -show_format -show_streams "$mkv_file" > "$json_file" 2>/dev/null; then
      echo "WARNING: Failed to create JSON metadata for $mkv_file"
    fi
...
create_access_files_only() {
    ...
    for mkv_file in "$rip_dir"/*.mkv; do
        ...
        base_name=$(basename "$mkv_file" .mkv)
        # Support both PM (preservation master) and VTS (Video Title Set) naming
        title_num=$(echo "$base_name" | grep -oE '(PM|VTS|A)[0-9]{2}' | grep -oE '[0-9]{2}')
